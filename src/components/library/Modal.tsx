...
const Modal = (props: ModalProps) => {
  ...
  useEffect(() => {
    if (props.type === "normal") {
      if (new URLSearchParams(location.search).get("type") !== null) {
        if (new URLSearchParams(location.search).get("type")?.toLowerCase().replace(/ /g, "-") !== props.title.toLowerCase().replace(/ /g, "-")) {
          setTypeCheck("none");
        } else {
   ...
   */
  return (
    <Card className="modal" id={props.title.toLowerCase().replace(/ /g, "-")} sx={{display: typeCheck}}>
      <CardContent>
